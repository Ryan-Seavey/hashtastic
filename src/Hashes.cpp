...
void StudentHash(const void * key, int len, uint32_t seed, void * out) {
  std::span<const uint8_t> temp(static_cast<const uint8_t*>(key), len);
	std::vector<uint8_t> funny{temp.begin(), temp.end()};
  funny.emplace_back(seed);




  *(uint32_t*)out = RyHash::hashTime(std::move(funny), 32);
